...
def load_ids(
    ...
    with open("origin_data/{}/entity2id".format(dataset), "r") as f:
        entity2id = json.load(f)
    with open("origin_data/{}/relation2ids".format(dataset), "r") as f:
        rel2ids = json.load(f)
    if dataset == "Wiki" or use_extra_descs == "_extra":
        with open(f"origin_data/{dataset}/{entity_text_file}", "r") as f:
            ent2entids = json.load(f)

        with open(f"origin_data/{dataset}/relationdescriptions", "r") as f:
...
def construct_rel_word(rel: str, dataset: str, use_extra_descs: bool, relation_descriptions : Dict[str, str]):
    ...
    if dataset == "NELL" and use_extra_descs != "_extra":
        ...
        if " ".join(res_[-2:]) == "in v":
            ...
            res_ = ["inverse"] + res_
        # import ipdb; ipdb.sset_trace()
        return correct_sentence(" ".join(res_)) + " " + relation_descriptions[rel]
...
def main(args):
    ...
    if dataset == "NELL":
        import ipdb; ipdb.sset_trace()
        relations = pd.read_csv("origin_data/NELL/NELL_relation_descriptions.csv", sep = "\t")
    ...
    entity2id, ents_to_ret, rel2ids, rels_to_ret = load_ids(
    ...
    # correct = np.loadtxt("origin_data/NELL/entity2vec.BERT")
    ...
    if ablation_remove is not None:
        ...
        assert use_extra_descs == ""
    np.savez(
        os.path.join("origin_data/", dataset, f"BERT_{use_extra_descs}_embed.npz"),
    ...
    if args.generate_for_new_relations is True:
        ...
        for task in ["train_tasks.json", "dev_tasks.json", "test_tasks.json"]:
            ...
            for relation in tasks.keys():
                tasks2ids[relation] = construct_rel_word(
                    relation, dataset=args.dataset, use_extra_descs=args.use_extra_descs
                )
        ...
        np.savetxt(
            "origin_data/{}/tasks2vec.BERT{}".format(dataset, use_extra_descs),
            task_embeddings,
        )
        with open("origin_data/{}/task2ids".format(args.dataset), "w") as f:
...
parser.add_argument(
    ...
    type=str,
    default=None,
    choices=[None, "type", "label", "description"],
    help="Argument used to facilitate generation of ablation study vectors. ",
)
