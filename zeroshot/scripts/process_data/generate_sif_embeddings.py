...
def main(args):
    ...
    # correct = np.loadtxt("origin_data/NELL/entity2vec.uSIF")
    ...
    if args.save_npz is True:
        np.savez(
            os.path.join("origin_data/", dataset, "SIF_embed.npz"),
            eM=entity_embeddings,
            rM=rel_embeddings,
        )
    else:
        np.savetxt(
            "origin_data/{}/entity2vec.uSIF{}".format(dataset, use_extra_descs),
            entity_embeddings,
        )
        np.savetxt(
            "origin_data/{}/relation2vec.uSIF{}".format(dataset, use_extra_descs),
            rel_embeddings,
        )
    if args.generate_for_new_relations is True:
        ...
        np.savetxt(
            "origin_data/{}/tasks2vec.uSIF{}".format(dataset, use_extra_descs),
            task_embeddings,
        )
