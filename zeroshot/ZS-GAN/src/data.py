...
def candidate_triples(dataset):
    ...
    with open(dataset + "/path_graph") as f:
        for line in f:
            line = line.rstrip()
            # e1 = line.split("\t")[0]  # noqa: F841
            # e2 = line.split("\t")[2]  # noqa: F841
    ...
    all_reason_relation_triples = (
        train_tasks.values() + dev_tasks.values() + test_tasks.values()
    )
...
def wiki_candidate(dataset):
    ...
    logging.info("How many few-shot relations", len(all_reason_relations))
    ...
    for key, triples in dev_tasks.items():
        # logging.info len(rel2candidates[key])
...
def combine_vocab(
    ...
    logging.info("LOADING SYMBOL2ID")
    rel2id = json.load(open(rel2id_path))
    ent2id = json.load(open(ent2id_path))

    # logging.info set(rel2id.keys()) & set(ent2id.keys()) # '' and 'OOV'
...
if __name__ == "__main__":
    ...
    combine_vocab(
        "Wiki/relation2ids",
        "Wiki/ent2ids",
        "Wiki/relation2vec.vec",
        "Wiki/entity2vec.vec",
        "Wiki/symbol2ids",
        "Wiki/symbol2vec.vec",
    )

    logging.info("Time clipse: ", time() - start)
