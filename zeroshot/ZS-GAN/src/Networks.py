...
import logging
from modules import LayerNormalization, SupportEncoder
...
class Extractor(nn.Module):
    ...
    def forward(self, query, support, query_meta=None, support_meta=None):
        ...
        """
        (
            query_left_connections,
            query_left_degrees,
            query_right_connections,
            query_right_degrees,
        ) = query_meta
        (
            support_left_connections,
            support_left_degrees,
            support_right_connections,
        ...
        support_neighbor = torch.cat(
            (support_left, support_e, support_right), dim=-1
        )  # tanh
...
class Generator(nn.Module):
    def __init__(self, args, ep_dim: int, input_dim=300, dropout=0.5):
...
class Discriminator(nn.Module):
    def __init__(self, ep_dim: int, dropout: float = 0.3):
        super(Discriminator, self).__init__()

        fc_middle = nn.Linear(ep_dim, ep_dim)
        self.fc_middle = spectral_norm(fc_middle)

        fc_TF = nn.Linear(ep_dim, 1)  # True or False
        self.fc_TF = spectral_norm(fc_TF)

        self.layer_norm = LayerNormalization(ep_dim)
...
if __name__ == "__main__":
    ...
    query_meta = [
        Variable(torch.ones(64, 4, 2).long()),
        Variable(torch.ones(64)),
        Variable(torch.ones(64, 4, 2).long()),
        Variable(torch.ones(64)),
    ]
    ...
    logging.info(query_g.size())  # (64L, 100L)
    logging.info(class_vec.size())
    ...
    logging.info(false.size())  # (1063L, 400L)
    logging.info(logits.size())  # (1063L, 51L)
    ...
    logging.info(logit_TF.size())  # (100L, 2L)
    logging.info(logit_class.size())  # (100L, 51L)
