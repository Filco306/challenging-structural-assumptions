        ...
        for i in range(k):
            ...
            for d in range(depth):
                if len(connections[node]) == 0:
                    break
                if d > 0:
                    # logging.info len(connections[node])
...
def combine_vocab(
    rel2id_path, ent2id_path, rel_emb, ent_emb, symbol2id_path, symbol2vec_path
):
    symbol_id = {}
    ...
    # logging.info symbol_id['PAD'] # PAD = 69557

    rel_embed = np.loadtxt(rel_emb)
    ent_embed = np.loadtxt(ent_emb)

    symbol_embed = np.concatenate(
        [rel_embed, ent_embed, np.zeros((1, rel_embed.shape[1]))]
    )
...
class Graph(object):
    ...
    def uni_search(self, node_pair, depth=3):
        ...
        for d in range(depth):
            ...
            for node in curr_layer:
                ...
                # logging.info 'fan-out: ', len(self.connections[node])
    ...
    def path_clean(self, path):
        symbols = path.split(" - ")
        # logging.info 'Uncleaned path: ', symbols
        entities = []
        ...
        # logging.info 'cleaned path: ', symbols
        return " - ".join(symbols)
    ...
    def pair_feature(self, pair, k=300, depth=3, mode="random"):
        ...
        """graphs = p.starmap(
            subgraph,
            [
                (pair[0], self.connections, k, depth),
                (pair[1], self._connections, k, depth),
            ],
        )"""
        # p.close()
        graphs = []
        ...
        for node in intermediate:
            left_ = left[node]
            right_ = right[node]
            combinations = product(left_, right_)
    ...
    def encode_path(self, path, seq_len=13):
        ...
        encoded = np.pad(
            encoded,
            (0, seq_len - len(encoded)),
            "constant",
            constant_values=(0, self.symbol2id["PAD"]),
        )
        return encoded
    ...
    def train_generate(self, few=5, batch_size=50, num_neg=1):
        ...
        logging.info("BUILD CANDAIDATES FOR EVERY RELATION")
        ...
        while True:
            ...
            if len(support_paths) == 0:
                logging.info("NO PATH FOUND, TRY AGAIN")
                continue
...
if __name__ == "__main__":
    combine_vocab(
        "NELL/relation2ids_fix",
        "NELL/ent2ids_fix",
        "NELL/relation2vec_fix.bern",
        "NELL/entity2vec_fix.bern",
        "NELL/symbol2ids_fix",
        "NELL/symbol2vec_fix.txt",
    )
