...
from data_loader import centroid_generate, Extractor_generate, train_generate_decription
import json
...
class Trainer(object):
    def __init__(self, args):
        ...
        if self.generate_text_embedding:
            if self.dataset in ["NELL", "Wiki"]:
                NELL_text_embedding(self.args, dataname=self.dataset)
            else:
                raise AttributeError("wrong dataset name!")

        rela_matrix = np.load(self.data_path + "rela_matrix.npz")["relaM"]
        logging.info("##LOADING RELATION MATRIX##")
        ...
        logging.info("##DEFINE FEATURE EXTRACTOR")
        ...
        logging.info("##DEFINE GENERATOR")
        self.Generator = Generator(self.args, ep_dim=self.ep_dim)
        self.Generator.cuda()
        ...
        logging.info("##DEFINE DISCRIMINATOR")
        self.Discriminator = Discriminator(ep_dim=self.ep_dim)
        ...
        _ = self.build_connection(max_=self.max_neighbor)
    ...
    def load_embed(self):
        ...
        if self.embed_model in [
            ...
            np.savez(
                "origin_data/" + self.dataset + "/Embed_used/" + self.embed_model,
                embeddings,
            )
            json.dump(
                symbol_id,
    ...
    def pretrain_Extractor(self):
        ...
        self.save_pretrain()
        logging.info("SAVE FEATURE EXTRACTOR PRETRAINING MODEL!!!")
    ...
    def train(self):
        ...
        if self.pretrain_feature_extractor:
            ...
            logging.info("Finish Pretraining!\n")
        ...
        for epoch in range(self.train_times):
            ...
            if epoch % self.loss_every == 0 and epoch != 0:
                ...
                logging.info(
                    "Epoch: %d, D_loss: %.2f [%.2f, %.2f, %.2f, %.2f], G_loss: %.2f [%.2f, %.2f, %.2f, %.2f]"  # noqa: E501
                    % (
                        epoch,
                        np.mean(D_losses),
                        D_screen[0],
    ...
    def eval_pretrain(self, mode="test", meta=True):
        ...
        test_candidates = json.load(
            open("./origin_data/" + self.dataset + "/" + mode + "_candidates.json")
        )
        ...
        logging.info("############   " + mode + "    #############")
        logging.info("HITS10: {:.3f}".format(np.mean(hits10)))
        logging.info("HITS5: {:.3f}".format(np.mean(hits5)))
        logging.info("HITS1: {:.3f}".format(np.mean(hits1)))
        logging.info("MAP: {:.3f}".format(np.mean(mrr)))
        logging.info("###################################")
...
if __name__ == "__main__":
    ...
    fname = f'logs/experiment_{args["prefix"]}_{datetime.now().strftime("%Y%m%d")}.log'
    print(f"outputting in {fname}")
