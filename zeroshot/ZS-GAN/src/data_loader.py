...
def Extractor_generate(
    dataset, batch_size, symbol2id, ent2id, e1rel_e2, few, sub_epoch
):
    ...
    while True:
        ...
        for _ in range(sub_epoch):
            ...
            if len(all_test_triples) < batch_size:
                query_triples = [
                    random.choice(all_test_triples) for _ in range(batch_size)
                ]
            else:
...
def centroid_generate(
    dataset, relation_name, symbol2id, ent2id, train_tasks, rela2label
):
...
def train_generate_decription(
    ...
    while True:
        ...
        for query in task_pool[: args["gan_batch_rela"]]:
            ...
            if len(candidates) <= 20:
                # logging.info 'not enough candidates'
                continue
        ...
        yield rela_matrix[
            rel_batch
        ], query_pairs, query_left, query_right, false_pairs, false_left, false_right, labels
