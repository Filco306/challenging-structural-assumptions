...
class Path(nn.Module):
    ...
    def __init__(
        ...
        self.symbol_emb = nn.Embedding(
            num_symbols + 1, input_dim, padding_idx=num_symbols
        )
        ...
        self.convs = nn.ModuleList(
            [nn.Conv2d(1, self.k_num, (k, input_dim)) for k in self.k_sizes]
        )
    ...
    def forward(self, path):
        ...
        convs = [
            F.relu(conv(path)).squeeze(3) for conv in self.convs
        ]  # every element (B, 100, W-(k-1))
...
if __name__ == "__main__":
    ...
    logging.info(support_encoder(support).size())
