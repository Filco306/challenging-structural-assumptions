...
1. Download [the ZS-GAN code from here](https://github.com/Panda0406/Zero-shot-knowledge-graph-relational-learning) for [ZS-GAN](https://arxiv.org/pdf/2001.02332.pdf).
2. Before running, there are two bugs in the original code that needs to be fixed. 
- First, Change line 71 in `data_loader.py` from 
```python3
...
if len(support_pairs) > 0:
     yield support_pairs, query_pairs, false_pairs, support_left, support_right, query_left, query_right, false_left, false_right
```
- Second, change line 508 in `trainer.py` from 
```python3
...
1. Download the [original code from here](https://github.com/genggengcss/OntoZSL). Follow their instructions, and make sure you can run their experiments. Some reminders here: 
