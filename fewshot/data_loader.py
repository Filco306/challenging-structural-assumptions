...
def train_generate(
    ...
    ent2id,
    e1rel_e2,
    use_task_rel_embeddings: bool = False,
    emb_model: str = None,
):
    logging.info("Loading Train Data")
    train_tasks = json.load(open("data/" + dataset + "/train_tasks.json", "r"))
    logging.info("Loading Candidates")
    rel2candidates = json.load(open("data/" + dataset + "/rel2candidates.json", "r"))
    if use_task_rel_embeddings is True:
        task2ids = json.load(open("data/" + dataset + "/task2ids", "r"))
        task_rel_embeddings = np.loadtxt(
            "data/{}/tasks2vec.{}".format(dataset, emb_model)
        )
    else:
        task2ids = None
        task_rel_embeddings = None
    ...
    while True:
        ...
        query_pairs = [
            [symbol2id[triple[0]], symbol2id[triple[2]]] for triple in query_triples
        ]
        ...
        yield (
            ...
            false_right,
            (
                torch.Tensor(task_rel_embeddings[task2ids[task_choice]])
                if use_task_rel_embeddings is True
                else None
            ),
        )
