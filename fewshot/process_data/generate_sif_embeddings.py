...
from src.utils import typed  # noqa: E402
...
def load_ids(
    ...
    with open("data/{}/ent2ids".format(dataset), "r") as f:
        ent2ids = json.load(f)
    with open("data/{}/relation2ids".format(dataset), "r") as f:
        rel2ids = json.load(f)
    if dataset == "Wiki" or use_extra_descs == "_extra":
        with open(f"data/{dataset}/{entity_text_file}", "r") as f:
            ent2entids = json.load(f)

        with open(f"data/{dataset}/relationdescriptions", "r") as f:
...
def construct_rel_word(rel: str, dataset: str, use_extra_descs: bool):
    if dataset == "NELL" and use_extra_descs != "_extra":
        ...
        ret.append(
            " ".join(
                (
                    ...
                    construct_rel_word(rel, dataset=dataset),
...
def main(args):
    ...
    # correct = np.loadtxt("data/NELL/entity2vec.uSIF")
    ...
    np.savetxt(
        "data/{}/entity2vec.uSIF{}".format(dataset, use_extra_descs), entity_embeddings,
    )
    np.savetxt(
        "data/{}/relation2vec.uSIF{}".format(dataset, use_extra_descs), rel_embeddings,
    )
    if args.generate_for_new_relations is True:
        ...
        np.savetxt(
            "data/{}/tasks2vec.uSIF{}".format(dataset, use_extra_descs),
            task_embeddings,
        )
        with open("data/{}/task2ids".format(args.dataset), "w") as f:
